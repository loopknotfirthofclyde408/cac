# ── templates: 写入 wrapper 和 fake-ioreg ──────────────────────
...
if ! (echo >/dev/tcp/"$_host"/"$_port") 2>/dev/null; then
    ...
    echo "[cac] 提示：运行 'cac check' 排查，或 'cacstop' 临时绕过" >&2
    exit 1
fi
...
export PATH="$CAC_DIR/fake-bin:$PATH"
...
_write_fake_ioreg() {
    mkdir -p "$CAC_DIR/fake-bin"
    cat > "$CAC_DIR/fake-bin/ioreg" << 'IOREG_EOF'
#!/usr/bin/env bash
CAC_DIR="$HOME/.cac"

# 非目标调用：透传真实 ioreg
if ! echo "$*" | grep -q "IOPlatformExpertDevice"; then
    _real=$(PATH=$(echo "$PATH" | tr ':' '\n' | grep -v "$CAC_DIR/fake-bin" | tr '\n' ':') \
...
EOF
IOREG_EOF
    chmod +x "$CAC_DIR/fake-bin/ioreg"
}
